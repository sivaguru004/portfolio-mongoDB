...
// Read More functionality for About Me section

const readMoreBtn = document.getElementById('readMoreBtn');
const moreAboutText = document.getElementById('moreAboutText');

if (readMoreBtn && moreAboutText) { // Check if elements exist before adding listener
  readMoreBtn.addEventListener('click', function(event) {
    event.preventDefault(); // Prevent default anchor link behavior
    if (moreAboutText.style.display === 'none') {
      moreAboutText.style.display = 'inline'; // Show the hidden text
      readMoreBtn.textContent = 'Read Less'; // Change button text
    } else {
      moreAboutText.style.display = 'none'; // Hide the text
